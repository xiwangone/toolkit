...
- 验证：对 <other-repo>（Kotlin）与 <repo>（多模块）实测通过
...
- ✅ 仓库工具链跨环境版 `scripts/git-tools/`（push/build/release/changelog/cleanup/pack，凭证按环境适配：凭证库 / Termux $GITHUB_TOKEN 或 SSH key）
- ✅ 跨 Agent 兼容：技能遵循 Agent Skills 标准（SKILL.md + frontmatter），多种 Agent 平台 均可直接使用
